...
class EmployeeCollector:
    ...
    def build_roster_from_slack(self) -> Dict[str, Dict]:
        ...
        try:
            ...
            if not slack_token:
                print(f"❌ No Slack bot token available")
    ...
    def get_all_calendar_ids(self, include_inactive: bool = False) -> Dict[str, str]:
        """
        Get comprehensive email to calendar_id mapping for company-wide collection
        
        Args:
            include_inactive: Whether to include inactive/removed employees
            
        Returns:
            Dictionary mapping email -> calendar_id (typically email == calendar_id)
        """
        print(f"\n📧 BUILDING COMPANY-WIDE CALENDAR ID MAPPING")
        print("-" * 48)
        
        # Get current roster data
        roster_data = self.to_json()
        employees = roster_data.get('roster_data', {}).get('employees', {})
        ...
        for email, employee_data in employees.items():
            # In Google Workspace, calendar_id typically equals email address
            calendar_id = employee_data.get('calendar_id', email)
            
            # Check if employee is active (has recent activity or multiple sources)
            sources = employee_data.get('sources', [])
            is_active = (
                len(sources) > 1 or  # Employee found in multiple systems
                'slack' in sources   # Employee active in Slack
            )
            
            if is_active:
                calendar_ids[email] = calendar_id
                active_count += 1
            elif include_inactive:
                calendar_ids[email] = calendar_id
                inactive_count += 1
        
        print(f"✅ Calendar IDs mapped: {len(calendar_ids)} total")
        print(f"  🟢 Active employees: {active_count}")
        if include_inactive:
            print(f"  🟡 Inactive included: {inactive_count}")
        else:
            print(f"  🟡 Inactive excluded: {inactive_count}")
        
        self._log_operation("calendar_id_mapping", {
            "total_mapped": len(calendar_ids),
    ...
    def get_active_calendar_ids(self) -> Dict[str, str]:
        """Get calendar IDs for active employees only (recommended for testing)"""
        return self.get_all_calendar_ids(include_inactive=False)
    
    def get_all_calendar_ids_including_inactive(self) -> Dict[str, str]:
        """Get calendar IDs for all employees including inactive (for production)"""
        return self.get_all_calendar_ids(include_inactive=True)
